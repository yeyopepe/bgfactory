# Modo `/ms-new todo <código>`

Procedimiento completo cuando la skill `ms-new` se invoca como `/ms-new todo <código>` (o el usuario pide explícitamente "convierte la idea `<código>` de todo en un change"). Esta entrada no nace de una petición nueva del usuario en el chat, sino del contenido ya apuntado por `ms-todo`.
