...
Fichero propio de `ms-version` (vive junto a `context.json` en `{workFolder}/framework/`, pero no forma parte de él): describe el procedimiento de shell/build concreto de este repo para generar el entregable jugable. Lo rellena `ms-version` la primera vez que se invoca y no existe todavía, preguntando al usuario; en invocaciones siguientes se lee y se sigue tal cual, sin volver a preguntar. También se actualiza cada vez que el usuario informa de un cambio en este procedimiento.
