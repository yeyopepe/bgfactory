...
- `skills.mockups` (opcional, por defecto `ms-internal-mockups-html`, no hace falta preguntar salvo que el usuario quiera usar otra skill/tecnología para generar las maquetas `design_*.html` de `ms-new`/`ms-fix`).
- `skills.diagrams` (opcional, por defecto `ms-internal-tech-mermaid`, no hace falta preguntar salvo que el usuario quiera usar otra skill/notación para generar los diagramas Mermaid de `ms-internal-workflow`/`ms-new`/`ms-fix`/`ms-how`).
