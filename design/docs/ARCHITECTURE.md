...
`createComponent()` inicializa `x`/`y` a `0` por defecto; al crear un componente desde el modo edición se le asigna una posición inicial que no se solapa con los componentes ya existentes.
...
- **`ui/componentRenderer.js`**: a diferencia de `ui/table.js`, sí conoce el modelo de componente. Expone `renderComponentsOnTable(worldEl, components, { onSelect, selectedId = null, onMove } = {})`, que limpia y vuelve a dibujar cada componente soportado (de momento solo `'cuadro-texto'`) sobre el `worldEl` de la mesa, posicionado según su propio `x`/`y`; si se pasa `onSelect`, la representación se vuelve clicable (invoca `onSelect(component)`, usado en modo edición para abrir la modal); si `component.id === selectedId`, se le añade la clase `text-box--selected` (contorno discontinuo de resaltado); si se pasa `onMove`, cada componente se vuelve arrastrable de forma individual (arrastre en pantalla convertido a coordenadas del mundo dividiendo por el zoom actual de la mesa), invocando `onMove(component, x, y)` al soltar con la posición final. Reutilizado por `modes/play/playMode.js` (sin `onSelect` ni `onMove`, solo lectura) y `modes/edit/editMode.js` (con `onSelect`, `selectedId` y `onMove`).
