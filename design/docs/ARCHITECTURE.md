...
```js
{
  ...
  moverEnModoJuego: boolean, // si el componente puede arrastrarse libremente en modo juego (false por defecto)
}
```

`createComponent()` inicializa `x`/`y` a `0` por defecto; al crear un componente desde el modo edición se le asigna una posición inicial que no se solapa con los componentes ya existentes. `width`/`height` se inicializan a `null` (tamaño automático según contenido) y solo se fijan explícitamente cuando el usuario redimensiona el componente desde modo edición (ver `ui/componentRenderer.js` en la sección 5). `moverEnModoJuego` se inicializa a `false` y solo es editable desde la pestaña "Generales" de `ui/componentModal.js`; controla si `modes/play/playMode.js` habilita el arrastre de ese componente en modo juego (ver sección 5, `ui/componentRenderer.js`).
