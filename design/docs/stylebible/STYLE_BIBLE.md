...
## 12.10 Grupo de botones de opción única (icono-solo)

Patrón para elegir exactamente una opción entre varias, cada una representada con un icono en vez de texto (`.align-group`/`.align-group__btn`, `ui/cardTextBoxModal.js`, cambio 00099 — alineación horizontal/vertical del texto dentro de un `TextBox` de carta): un contenedor `.align-group` (`display:flex; gap:0.25rem`) con un `.align-group__btn` por opción (botón cuadrado `32×32px`, icono SVG centrado con `stroke="currentColor"`, `title`/`aria-label` como etiqueta accesible al no llevar texto — mismo criterio de "botón icono-solo" de la sección 9). Estado reposo: fondo `var(--bg-subtle)`; hover: `var(--bg-hover)`; opción activa (`.align-group__btn.active`): fondo `var(--accent-blue)`, texto/icono `var(--text-light)` — mismo lenguaje visual que `.modal__tab.active` (sección 7/9), adaptado a botón cuadrado icono-solo en vez de pestaña con texto. Al pulsar un botón del grupo, se actualiza el dato asociado y se recalcula `active` en los tres botones del grupo (nunca más de una opción activa a la vez).

Distinto de un `<select>` nativo (la opción activa se ve destacada visualmente sin desplegar nada) y de un checklist (sección 12.5, que permite selección múltiple). Cualquier elección única futura representada con iconos debe reutilizar este patrón en vez de crear uno ad-hoc — mismo criterio que ya se sigue con `.resize-handle` (sección 11), `.help-icon` (sección 12) o `.resource-add__menu` (sección 12.7).
