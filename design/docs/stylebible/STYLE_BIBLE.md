...
**Segundo manejador en la esquina superior izquierda (cambio 00128):** `.resize-handle--tl`, aplicada además de `.resize-handle` sobre el mismo host (mismo mecanismo de `ui/resizeHandle.js`, con el parámetro `corner: 'tl'` — no es un segundo patrón de redimensionado, es el mismo manejador anclado a la esquina opuesta, decidido explícitamente como excepción a la restricción anterior). Cualquier elemento redimensionable de la app puede tener este segundo manejador, no solo un subconjunto. Diferencias respecto a `.resize-handle`: posición `left: 0; top: 0` en vez de `right: 0; bottom: 0` (mismo tamaño de contenedor y de grip, mismo `::after` con gradientes, mismo aspecto en reposo/`:hover`/`.resize-handle--active`, mismo cursor `nwse-resize` — ambas esquinas están sobre la misma diagonal). Al arrastrarlo, la esquina inferior derecha del elemento queda fija (el manejador existente sigue ejerciendo de ancla en la esquina superior izquierda); es responsabilidad de quien llama a `attachResizeHandle` aplicar también el desplazamiento de posición (`dx`/`dy` que expone `corner: 'tl'`) sobre el modelo del host, no solo el tamaño.
...
Usos de este patrón (cambio 00072 salvo el indicado):
...
- `ui/cardShapeModal.js` (figura geométrica de una carta, cambio 00120): "Borde" (des/activador: checkbox "Activar borde" + color/grosor, grosor 1–20 — mismo patrón que `TextBox`, sin ninguna diferencia entre ambos; una figura nueva nace con el checkbox marcado) y "Fondo" (informativo, mismo criterio que `TextBox`).
...
**Segundo uso del patrón (cambio 00118):** `ui/cardEditorModal.js` reutiliza tal cual las mismas clases (`resource-add`/`resource-add__button`/`resource-add__menu`/`resource-add__item`/`resource-add__item-label`) para el botón "Añadir elemento" de cada cara del editor de cartas (opciones: Imagen de fondo / Cuadro de texto / Figura geométrica), en vez de la fila de botones de texto independientes que tenía antes — confirma que el patrón es agnóstico al dominio, no exclusivo de subir recursos a la galería.

**Figura geométrica del editor de cartas (cambio 00118):** `.card-editor-modal__shape` (`ui/cardEditorModal.js`) es un bloque hermano de `.card-editor-modal__textbox` — mismo cursor `move`, mismo contorno discontinuo azul en `:hover` y mismo contorno continuo `--selected` al seleccionarla, y el mismo `.resize-handle` (sección 11) en su esquina inferior derecha. No lleva tipografía ni contenido de texto propio: solo `border-radius` (`50%` si la figura es circular/elíptica, `0` si es cuadrada), `background-color` y `border` (línea simple, sin el bisel reservado a `'tablero'`/`'dado'`, sección 13; visible solo si `bordeActivo` está activo — ver sección 12.6 sobre `ui/cardShapeModal.js`, cambio 00120). El redimensionado libre en ambos ejes con Shift forzando 1:1 para el tipo circular/elíptico reutiliza el comportamiento genérico ya existente en `ui/resizeHandle.js` para `axis: 'both'` (el mismo que usa la proporción "Circular" de `'carta'`), sin necesitar ningún `clamp` de proporción.
...
**Esquinas redondeadas de "Carta" (cambio 00053; configurable desde el cambio 00117):** el tipo de componente `'carta'` (`ui/componentRenderer.js`, y `.carta` en `main.css`) usa como base `var(--radius-lg)` en la clase CSS, el radio que la sección 5 reserva para "contenedores destacados" (el que también usa `.modal`, los paneles flotantes, y la propia `.carta`) — no es un valor especial. Para las cinco proporciones rectangulares/cuadrada, ese valor (`8px`, equivalente a `var(--radius-lg)`) es ahora el resultado por defecto de la propiedad `esquinasRedondeadas` (booleana, ver `ARCHITECTURE.md` sección 4, tipo `'carta'`), aplicado como estilo inline sobre el elemento (`getCartaShapeCss`, `core/cardProportions.js`) y por tanto con prioridad sobre la clase: desmarcar el checkbox "Esquinas redondeadas" de `ui/cardEditorModal.js` (toolbar, junto al selector de Proporción) aplica `border-radius: 0` en su lugar. Circular y Hexagonal no se ven afectadas, mantienen su recorte fijo (`50%`/`clip-path`). No se introduce ningún patrón visual nuevo: ese checkbox reutiliza tal cual `.modal__field--checkbox`, ya usado en otros checkboxes de la app (p. ej. "Bloqueado"/"Oculto" en `ui/componentModal.js`). Desde el cambio 00063 la carta también lleva sombra de contacto nivel 1, igual que el resto de piezas de juego (sección 6), sin verse afectada por esta propiedad.
...
**Forma circular de "Mazo" (cambio 00121):** igual que la proporción `'circular'` de "Carta" (párrafo de esquinas redondeadas, más abajo), la propiedad `forma` de `'mazo'` fija `border-radius: 50%` inline cuando vale `'circular'`, con prioridad sobre el `var(--radius-lg)` que la clase `.carta` aplica por defecto — mismo mecanismo, no una excepción nueva. Se aplica tanto a la caja del mazo como a su contenido interior recortado (reverso de la carta de arriba, o el icono de "mazo vacío"). La sombra de contacto no necesita ningún tratamiento especial (a diferencia de las siluetas hexagonales de "Carta"): `box-shadow` ya sigue el `border-radius` del elemento, así que una caja circular proyecta automáticamente una sombra circular sin pasar a `filter: drop-shadow`. La "zona de revelado" (`.mazo-reveal-zone`, párrafo anterior) adopta el mismo criterio: `border-radius: 50%` inline cuando el mazo es circular, en vez de su `var(--radius-sm)` por defecto.
