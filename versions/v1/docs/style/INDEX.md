...
- `'dado'` (`ui/componentRenderer.js`, `renderDiceSilhouette`): misma familia de recurso — copia de la silueta frontal en tono oscuro derivado, desplazada detrás (profundidad), más contorno fino y líneas internas de faceteado (4/8/9+ resultados), todo con `shadeColor`. Su sombra de contacto (`.dice`) usa `filter: drop-shadow` (silueta no rectangular).
