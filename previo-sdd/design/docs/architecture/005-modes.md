...
- `ui/editModeToggle.js` implements an enter/exit flow (not a two-option selector) over `core/state.js` (`mode: 'play' | 'edit'`): `renderModeSwitcher` (play mode, `#mode-switcher`: "Importar" button + "Entrar en modo edición" button) and `renderEditToolbar` (edit mode, `.edit-toolbar` strip under the header). Both operate over `setMode()`/event `mode:changed` of `core/state.js`, with no changes in that layer.
...
- "Importar" is **not** exclusive to `.edit-toolbar` any more: `renderModeSwitcher` prepends it to `#mode-switcher` (play mode), left of "Entrar en modo edición" (order: `Importar | Entrar en modo edición`, then the floating `.mode-switcher__fit-btn`). Both entry points build their controls with the module-internal `createImportControls({ buttonClassName? })` (hidden `<input type="file" accept=".json">` + button, `change` → `importComponentsFromFile`) — one import flow, two call sites, no duplication. `buttonClassName` is passed only by the play-mode call (`'mode-switcher__import-btn'`, for icon styling — see `../style/002-componentes-layout.md`); the edit-mode call passes nothing and the button keeps `.edit-toolbar button`'s look.
