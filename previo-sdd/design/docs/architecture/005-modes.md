...
- `ui/editModeToggle.js` implements an enter/exit flow (not a two-option selector) over `core/state.js` (`mode: 'play' | 'edit'`): `renderModeSwitcher` (**runs in both modes** since 00244 — no early return). Operates over `setMode()`/event `mode:changed` of `core/state.js`, with no changes in that layer.
- `main.js` mounts `renderAppTitle(#app-title)` + `renderModeSwitcher(#mode-switcher)` on every render — `h1#app-title` and `#mode-switcher` are the only two chrome containers in `index.html` (00290: `#edit-toolbar` removed, its content merged into `h1#app-title`; see [`006-ui-layer.md`](006-ui-layer.md)).
- **`#mode-switcher` content (00244 reorg — canonical detail in [`006-ui-layer.md`](006-ui-layer.md), `ui/editModeToggle.js`):**
  ...
  - edit mode: `[Modo] [Ajustar zoom] [Configuración]` only (the file block lives inside `h1#app-title`, 00290 — see below).
- **Import/Export in edit mode** (00290): rendered inside `h1#app-title` by `ui/appTitle.js#renderControls`, not by `ui/editModeToggle.js` — merged into the title's own element so the header's textured background is a single continuous layer (canonical detail in [`006-ui-layer.md`](006-ui-layer.md)).
- `createModeButton()` — mode-switch button, class `mode-switcher__mode-btn`, primary-action blue, always in `#mode-switcher`: text `t('toolbar.modeEdit')` ("Modo Edición") in play mode, `t('toolbar.modePlay')` ("Ir al Modo Juego", since 00283) in edit mode. Toggles via `setMode`.
- `createImportControls()` (no `buttonClassName` param any more) + `createExportMenu()` — "Importar"/"Exportar", same "ghost on dark" look wherever mounted (`#mode-switcher` in play mode, `h1#app-title` in edit mode); hidden `<input type="file" accept=".json">` + button, `change` → `importComponentsFromFile`. One import flow, two call sites.
...
- "Ajustar zoom" button (`createFitButton(className)`, action `fitToBounds(getComponentsBounds(getComponents()))`): icon-only, class `.mode-switcher__fit-btn`, in `#mode-switcher` in **both modes** (since 00244). Identical look/position across modes (see `../style/002-componentes-layout.md`, §9-§10).
