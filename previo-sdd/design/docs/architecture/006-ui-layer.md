...
- **`ui/editModeToggle.js`**: exposes `renderModeSwitcher` ("Entrar en modo edición" button) and `renderEditToolbar` (fixed "Salir del modo edición", "Exportar"/"Importar" bar — see `007-persistence-build.md`). Both add, at the right end of their container, an icon-only "Ajustar zoom" button (same SVG in both places), the handler computes `getComponentsBounds` and calls `fitToBounds` of `ui/table.js`. "Exportar"/"Importar" collect the file name and selection through their own modals, not a native `prompt()` (see below).
