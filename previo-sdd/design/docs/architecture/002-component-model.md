...
| `profundidad` | number | `0` (`4` on a new `'dado'`) | Thickness in px of the visual extrusion (stacked solid layers, `box-shadow`/`filter: drop-shadow` by type). Cap `40`. `0` = no effect. No effect on `'texto'` whatever its value. See `ui/componentRenderer.js` (`buildExtrusionLayers`) and `../style/001-tokens-visual.md` (Elevation) | "Visuales" tab of `ui/componentModal.js`, "Profundidad" field (`<input type="number">`, clamp `[0, 40]`) |
