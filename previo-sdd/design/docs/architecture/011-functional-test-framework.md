...
| `functional/text-box-component.test.js` | 017 | state + ui — `createDefaultComponent('texto')` applies no special defaults (`properties` empty, `width`/`height` null, unlike 'dado'/'tableroSimple'); the documented defaults (font size 16, black text, transparent background) are `ui/componentModal.js` fallbacks applied when rendering the field, not the created model. [gotcha] the "Generales" tab's tooltip `<textarea>` coexists in the DOM with the "Específicas" tab's content `<textarea>` (tabs only toggle `display`) — locate by `rows === 3`, not by document order |
