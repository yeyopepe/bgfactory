...
| `src/test/runner-page.html` | Browser (headless) | Page loaded per test file. Replicates the 4 containers of `src/index.html` (`#app-title`, `#mode-switcher`, `#content`, `#app-version` — `#edit-toolbar` removed 00290, Import/Export now render inside `#app-title`) + `<script type="application/json" id="initial-state">`. Inline `<script type="module">` reads `?file=`, imports `./harness.js`, dynamic-imports the test file, runs `harness.run()`, publishes results on `window`. Does NOT load `main.js` or `styles/main.css`. |
| `src/test/runner-page-boot.html` | Browser (headless) | Replica of `runner-page.html` that **does** load `src/main.js`, once, for end-to-end boot cases. Same 4 containers + `#initial-state`. Inline `<script type="module">`: dynamic-imports the test file, `await mod.setupBoot?.()` (prepares `localStorage` / `#initial-state`), `await import('../main.js')` (real bootstrap, once), `await mod.afterBoot?.()` (test registers its `it()` now that `main.js` has run), then `harness.run()`. Same `window` contract as `runner-page.html`. Used only for `*.boot.test.js` files (routed by `run.js`). Does NOT load `styles/main.css`. |
...
mountChrome()                    renderAppTitle(#app-title) + renderModeSwitcher(#mode-switcher) (00290: no more
                                 renderEditToolbar/#edit-toolbar, removed — Import/Export in edit mode render
                                 inside #app-title now)
...
| `functional/tag-panel.test.js` | 008 | state + ui — `core/tag.js` (`isTagNameTaken` normalization, `getComponentsUsingTag`); `ui/tagList.js`/`ui/tagModal.js`: add from panel and on-the-fly, click-row selects all members (replacing prior selection) including group-tagged members, usage counter, delete with/without usage (`ui/tagDeleteConfirmModal.js`), per-element "Sacar" from the edit window. [gotcha] the background tag panel only re-renders on its own explicit actions (no `eventBus` listener) — a mutation from inside an open modal (e.g. "Sacar") is not reflected in the panel's DOM until it's remounted |
| `functional/resource-upload-batch.test.js` | 009 | ui — the 3 hidden `<input type=file>` of `modes/edit/editMode.js` (`loadResourceBatch`, not exported) driven via real `File`/`DataTransfer` + `change` event (no prior helper in `helpers.js` for this); single invalid file blocks with error, batch/folder upload skips invalid and summarizes (`ui/batchUploadSummaryModal.js`), folder with nothing valid warns, in-batch duplicate names group into one `ui/resourceReplaceConfirmModal.js` confirmation |
| `functional/resource-webp-conversion.test.js` | 010 | state — `core/imageConversion.js#convertImageToWebP(file, dataUrl)` in isolation, no UI: PNG/JPG/JPEG converted, WebP/SVG/GIF passthrough, conversion failure falls back to original |
| `functional/board-image-modal.test.js` | 011 | ui — `ui/boardImageModal.js#openBoardImageModal` (shared "Elegir imagen" picker used by board background and card faces): search box gated by ≥1 image, live accent/case-insensitive filter, empty-filter message, filtered-out selection still applied on Accept, search resets per invocation |
| `functional/text-box-component.test.js` | 017 | state + ui — `createDefaultComponent('texto')` applies no special defaults (`properties` empty, `width`/`height` null, unlike 'dado'/'tableroSimple'); the documented defaults (font size 16, black text, transparent background) are `ui/componentModal.js` fallbacks applied when rendering the field, not the created model. [gotcha] the "Generales" tab's tooltip `<textarea>` coexists in the DOM with the "Contenido" tab's content `<textarea>` (tabs only toggle `display`) — locate by `rows === 3`, not by document order |
| `functional/global-shortcuts.test.js` | 028 | ui — `ui/globalShortcuts.js#initGlobalShortcuts` mounted directly on `document`, combined with real modals (`ui/tagModal.js`/`ui/errorModal.js`) for ESC/INTRO/SUPR cases; arrow keys via injected `onMoveSelected`/`isEditMode` callbacks |
| `functional/splash-screen.test.js` | 041 | ui — `ui/splashScreen.js#showSplashScreen()` mounted directly on `document.body`; DOM structure, `mockRandom` over the 4 logo variants, link does not close the overlay, real `SPLASH_DURATION_MS` (3000ms) wait for auto-dismiss |
