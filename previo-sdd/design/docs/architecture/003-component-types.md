...
> Catalog of every type-specific property as it appears in the modal's "Contenido" tab (order, sections, visibility condition, sub-modals): see the functional entry "Catálogo de propiedades de componentes, grupos y etiquetas" (`../features/040-catalogo-de-propiedades-de-componentes-grupos-y-etiquetas.md`).
...
### Pre-defined sets (bulk creation outside the standard flow)

`ui/componentTypeModal.js` (00236) adds a second section below the standard type list: `.component-type-modal__preset-item`, an immediate-action row (no radio, does not go through `onAccept`/"Aceptar") that invokes `openComponentTypeModal({ onPresetSelected })`'s `onPresetSelected(presetId)` callback. `[gotcha]` (00275) does NOT close the modal directly anymore — `openComponentTypeModal` returns `{ close }`, and the modal stays open until whoever orchestrates the preset (`editMode.js#openAddModal`) decides to call it.

`editMode.js#openAddModal` wires the first preset (`'frenchDeck54'`) to: `getFrenchDeckPresetSummary()` → `ui/presetConfirmModal.js#openPresetConfirmModal({ summary, onAccept })` (`../style/003-modales-menus.md`, "Preset confirmation window") → on accept, `typeModal.close()` then `runWithProgressModal(text, () => createFrenchDeckPreset({ deckId, cardPrefix }))`. On cancel, nothing runs and the type modal stays open (no `onCancel` wired for this preset — the confirmation window's own default of just closing itself is enough).

First (and, for now, only) preset: `core/presets/frenchDeck.js#createFrenchDeckPreset({ deckId?: string, cardPrefix?: string } = {})` — builds 55 resources (54 unique card faces + 1 shared back, from `core/svgTemplates.js`'s pure SVG-string generators and `data/cardTemplates.js`'s fixed catalog/templates), 54 `'carta'` components (`createDefaultComponent('carta')`, `caraFrontal`/`caraTrasera.imagenResourceId` set, `caraActual: 'frontal'`, grid `x`/`y`, `groupId` left at its `createDefaultComponent` default of `null` — the 54 cards are created loose, not grouped, since 00271) and 1 `'mazo'` component (`cartaIds` = the 54 card ids). `[gotcha]` (00275) `deckId`/`cardPrefix` are optional: an empty/omitted value falls back to the same fixed defaults as before (`t('componentTypeModal.preset.frenchDeck.deckName')` / `'card-'`) — `data/cardTemplates.js#cardId({ suitId, rank, jokerIndex, prefix = 'card-' })` gained the `prefix` param for this; `buildFrenchDeckCatalog()` itself still calls `cardId()` unprefixed (its own internal use for counting/positioning is unaffected), `createFrenchDeckPreset` re-derives each card's id with the user's `cardPrefix` from the same `suitId`/`rank`/`jokerIndex` the catalog entry carries. `core/presets/frenchDeck.js#getFrenchDeckPresetSummary() -> { title, defaultDeckId, defaultCardPrefix, counts: [{ icon, count, label }] }` derives the 3 summary rows from `buildFrenchDeckCatalog().length`, no SVG generation, no state mutation — feeds `ui/presetConfirmModal.js`.

Bulk creation mechanism, outside `componentTypeModal.js`/`componentModal.js`'s normal one-at-a-time flow: builds every entity in memory first, then a single `loadResources(...)`/`loadComponents(...)` call each (not N `addResource`/`addComponent` calls) — same "full replacement + 1 event" mechanism `main.js#bootFromSeedOrDefaults()` already uses at startup. `loadComponents` normalizes `order` on its own (`compactOrders`), so the preset does not assign it by hand. Starting point for any future pre-defined set: reuse this same mechanism, the modal's immediate-action item pattern (`../style/003-modales-menus.md`, "Pre-defined set item (immediate action)"), and the confirmation-window orchestration above (own `getXSummary()` + `openPresetConfirmModal`) — both are generic, not French-deck-specific.
