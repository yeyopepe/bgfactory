...
- **Same control, two looks by host bar** — "Importar" (`createImportControls`, `ui/editModeToggle.js`). One control, two mount points: `.edit-toolbar` (edit mode) → transparent/border style of `.edit-toolbar button`, no extra class; `#mode-switcher` (play mode) → primary blue of `#mode-switcher button`, class `mode-switcher__import-btn`. No shared class forces one look — each button inherits its host bar's button scheme.
  - `#mode-switcher .mode-switcher__import-btn`: `display: inline-flex; align-items: center; gap: 0.375rem` + `.icon-frame` at `16×16; display: block` — an explicit icon+text rule, because `#mode-switcher button` (unlike `.edit-toolbar button`) carries neither `inline-flex` nor an `.icon-frame` size. Analogous to `.edit-toolbar button .icon-frame { 16×16 }`. Background/padding/radius/font-size and `:hover { opacity: 0.9 }` all inherited from `#mode-switcher button` / `#mode-switcher button:hover`.
  - `#mode-switcher` flex `gap: 0.5rem` spaces "Importar" from "Entrar en modo edición"; DOM order = visual order (`Importar` prepended, then "Entrar en modo edición", then the floating `.mode-switcher__fit-btn`).
