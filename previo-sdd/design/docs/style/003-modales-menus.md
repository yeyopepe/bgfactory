...
- **Interaction**: `role="button" tabindex="0"`, `[click]` and `[keydown: Enter | Space]` both invoke the same handler (`preventDefault` on the key case), never touching the row-owning list's `selectedType` state nor the modal's own "Aceptar"/"Accept" footer button (that button keeps applying only to whichever ordinary row's radio is checked). `[gotcha]` (00275) does NOT call `overlay.remove()` — it opens "Preset confirmation window" below and leaves this modal open behind it; `openComponentTypeModal` returns `{ close }` for the orchestrator to close this modal only once the confirmation window is accepted (`modes/edit/editMode.js#openAddModal`).
...
## Preset confirmation window

Pattern for a confirmation step between selecting a "Pre-defined set item" (above) and actually creating its components: `ui/presetConfirmModal.js`, `openPresetConfirmModal({ summary, onAccept, onCancel })` (00275). Generic entry point for any pre-defined set, not just the French deck — a preset module supplies `summary` (see below), this modal renders it and collects the two editable identifiers.

- Reuses `.modal-overlay`/`.modal`/`.btn-cancel`/`.btn-accept` — no new modal skeleton.
- `summary: { title, defaultDeckId, defaultCardPrefix, counts: [{ icon, count, label }] }` — `label` embeds `{count}` at its start (e.g. `"54 recursos nuevos"`); the modal splits the leading digits into `.preset-confirm-modal__summary-count` and renders the rest as plain text.
- **Summary list** (`.preset-confirm-modal__summary`, one `.preset-confirm-modal__summary-item` per `counts` entry): read-only rows, same criterion as `.context-menu__info-row` — no hover, no `cursor: pointer`, no accent background/border (deliberately distinct from `.component-type-modal__preset-item`'s accented row look above). Each row: decorative icon (`.preset-confirm-modal__summary-icon`, `aria-hidden`, `ICON_SIZE.menu` via `ui/icons.js`) + `.preset-confirm-modal__summary-count` (`font-weight: var(--font-semibold)`, `color: var(--accent-blue-dark)`) + label text. `border-bottom: 1px solid var(--border-neutral)` between rows, none on the last.
- **Editable identifiers**: `<fieldset class="modal__section">` ("Sections inside property tabs" above) with two `.modal__field` text inputs, preloaded with `summary.defaultDeckId`/`summary.defaultCardPrefix`. `[gotcha]` an emptied field (after `.trim()`) falls back to its default value on Accept instead of blocking confirmation — no live validation, no error state.
- **Accept**: reads both inputs, `.trim()`s them, closes this overlay, then calls `onAccept({ deckId, cardPrefix })` — the caller (not this modal) is responsible for closing "Pre-defined set item"'s modal and running the actual creation (typically inside "In-progress operation modal").
- **Cancel**: calls `onCancel` if given, closes only this overlay — the modal behind ("Pre-defined set item"'s list) stays open.
- Any future pre-defined set needing a confirmation step before creation: reuse `openPresetConfirmModal` with its own `summary` instead of an ad-hoc confirmation dialog.
...
|---|---|---|
| `.component-editor-modal` | Component editing (`ui/componentModal.js`, `openComponentModal`) | `width: clamp(460px, 50vw, min(600px, 65vw))` — recomputed dynamically on window resize with no JS. **Vertical position** (00284): `position: fixed` (not `.modal-overlay`'s flexbox centering) with `left: 50%; transform: translateX(-50%)` for horizontal centering, and `top` set in JS (`openComponentModal`), fixed across its 5 tabs instead of recentering per tab. `top` = `max(0, (innerHeight - min(maxTabHeight, innerHeight * 0.8)) / 2)`, where `maxTabHeight` is `modal.offsetHeight` measured once right after DOM insertion by temporarily toggling each tab's `content.style.display` to `'block'` in turn (same "measure via temporary visibility" technique as `measureTextoNaturalSize()` in the same file) — the `innerHeight * 0.8` cap mirrors `.modal`'s existing `max-height: 80vh`, so a tab taller than that keeps the same `overflow: hidden` + internal `.modal__content` scroll as before. Recalculated on `window`'s `resize` event (listener added on open, removed via `closeModal()` on every close path — delete/cancel/accept/click-outside). [gotcha] the anchor is **not necessarily the visually tallest tab by content amount** — it's whichever tab measures the tallest `modal.offsetHeight` for the specific component being edited (e.g. a `'carta'` with many tags/copies can make "Generales" taller than "Apariencia"); the position is always correct because it's measured per opening, not assumed. Same `position: fixed` + `resize`-listener pattern as `.card-editor-modal`'s manual-resize below (`../architecture/006-ui-layer.md`, "Window sizing" → Layer 1), applied here to plain positioning instead of manual sizing. |
...
### Property-tab icons (`.modal__tab`, 00269)

Each of the 5 `ui/componentModal.js` property tabs shows an icon (`ICON_SIZE.menu`, 16px) to the left of its label, same visual criterion as "Floating-panel header: icon + title + count badge" above (icon + text, `gap: var(--space-2)`).

- `.modal__tab` is `display: flex; align-items: center; gap: var(--space-2)` (was text-only before 00269); each tab's icon is wrapped in its own `<span class="modal__tab-icon">` sibling to a `<span>` holding the label text — not the SVG as a direct child of the button — so the icon's color can be set independently of the label's on the active tab (see below).
- Icon per tab, by internal id: `general` → `settings`, `visual` → `tab-visual` (paintbrush), `specific` → `type-carta` (reused — the "Contenido" tab, formerly "Específicas", shares its icon with the "Carta" component-type icon), `interacciones` → `tab-interacciones` (zap), `copias` → `clone` (reused, same icon as the "Copia"/"Tiene copias" indicators, "Component context menu" section below).
- Icon reuse criterion: when an existing icon already represents the same concept elsewhere in the app (`settings`, `type-carta`, `clone`), the tab references it directly by name instead of duplicating the same SVG string under a second key in `ui/icons.js`'s `ICONS` map.
- Active-tab color (00269): besides the existing blue bottom border and dark label text, `.modal__tab.active .modal__tab-icon` sets `color: var(--accent-blue)` — the icon's `<span>` wrapper is what makes this possible, since the SVG's `stroke="currentColor"` would otherwise also recolor the label if both were siblings sharing the button's own `color`.
- Widened minimum modal width: with an icon + `gap` added to each of the 5 tabs, `.component-editor-modal`'s `clamp()` minimum grew from `400px` to `460px` ("Wide modals" above) so the row keeps fitting on one line at the modal's narrowest width, with no tab text wrapping.
...
- `ui/componentModal.js`, type `'tableroSimple'`: "Efecto" (informational, i18n `common.visual`: "Biselado en el borde" checkbox field checked by default, see `004-naming-and-patterns.md` (What NOT to do) and "Bevel/depth" below; "Sombra" checkbox field checked by default, see `001-tokens-visual.md` (Elevation)) — last of the ordered group, after "Extrusión"; "Borde" (en/disabler: "Activar borde" checkbox + color/thickness 1–20, same pattern as `TextBox`/`Shape` — a new or old board starts with the checkbox checked) — before "Extrusión" in the ordered group; and untitled (`.modal__section--untitled`) the "Fondo" field in the "Contenido" tab (a selector with three options: "Color y patrón"/"Imagen"/"Color").
- `ui/componentModal.js`, type `'tableroPersonalizado'`: "Efecto" (informational, same "Biselado en el borde" and "Sombra" fields as `'tableroSimple'`, same position in the ordered group), followed in the "Contenido" tab by the "Editar diseño del tablero" button (no own `.modal__section`, a loose field).
...
- **`.app-title--hoverable`** (edit mode, not editing): a modifier on `.app-title__row` (00283 — was on `h1` directly before the header restructured to hold the mode indicator below the title) — `cursor: pointer` (convention of "Cursors", no own specific cursor), a pencil icon (`.app-title__pencil`, inline SVG `stroke="currentColor"`) hidden by default (`opacity: 0`), shown only on `:hover` (`opacity: 0.85`, transition `var(--transition-fast)`) — never permanently visible (unlike the lock/hidden badges of "Component identifier label"). [gotcha] the click handler that starts editing is bound to `h1` itself, not to `.app-title--hoverable`'s own element — clicking anywhere inside `h1` (title row, mode indicator, or the controls row below) would open the inline editor unless that other element stops the click from bubbling — see `.app-title__controls`'s `stopPropagation` below.
- **`.app-title--editing`** (edit mode, editing): modifier on `.app-title__row`, replaces the text with `.app-title__input` — an `<input type="text">` with custom styling (not the generic form-field one, to keep the row's own size/typography: `font: inherit`, background `rgba(255,255,255,0.08)` over the header's dark gradient, border `2px solid var(--accent-blue)`, text `var(--text-light)`) — followed by `.app-title__version`, the version in `var(--text-muted)`, no interaction, outside the `<input>` itself.
- In play mode: the `h1` carries none of these classes and has no children besides its own text node — it behaves as the generic `h1` (`001-tokens-visual.md`, Typography), with no special cursor, icon, or indicator.
...
### Header layout in edit mode: title + mode indicator + file controls (00283, extended 00290)

In edit mode, `h1#app-title` holds a single child `div.app-title__block` (`display: flex; flex-direction: column; gap: 0.15rem`) instead of its title content directly:

| Element | Content | Notes |
|---|---|---|
| `.app-title__block > .app-title__row` | The title itself (hoverable or editing sub-state, see above) | `display: flex; align-items: center; gap: 0.5rem` — same row layout `h1` used to have directly |
| `.app-title__block > .app-title__mode-indicator` | `<span>`, text = `t('toolbar.modeEdit')` ("Modo Edición" / "Edit Mode") | `font-size: var(--text-md)` (18px — one step below the title's `var(--text-lg)`, one step above default UI text `var(--text-sm)`), `font-weight: 600`, `color: var(--accent-blue)`, `letter-spacing: 0.02em` |
| `.app-title__block > .app-title__controls` | Import/Export buttons (00290) | `display: flex; align-items: center; justify-content: flex-end; gap: 1rem` — transparent, no own background/shadow (inherits `h1`'s); buttons use the same "ghost on dark" scheme as `#mode-switcher`'s file block (`002-componentes-layout.md`, "Header control row") |

`h1` itself switches from a fixed `height: 3.5rem` to `min-height: 3.5rem` (00283) so it can grow to fit the (now three-row) block in edit mode while staying single-line height in play mode; `display: flex; align-items: center` on `h1` is unchanged and still centers the (now potentially taller) content vertically.

- `.app-title__controls`'s buttons open the file picker / the Export dropdown, same as `#mode-switcher`'s equivalents in play mode — no behavioral difference by host container.
- [gotcha] `.app-title__controls` has its own `click` listener calling `event.stopPropagation()`. `h1.onclick` (bound by `renderHoverable`, see "Editable header title" above) fires on any click landing anywhere inside `h1` unless something stops it first — without this guard, clicking "Importar"/"Exportar" would also open the title's inline editor as an unwanted side effect.
- [gotcha] `.app-title__block` (and its three row children) must be inserted into `h1` (and therefore into the document) **before** the editing sub-state calls `input.focus()` — a detached element never becomes `document.activeElement`, silently breaking the confirm-on-blur/Enter flow. Not a CSS concern by itself, but the reason the mode indicator and controls row are appended before the title row's content is rendered, not after.
- Never present in play mode — no residual node, no residual class. Import/Export reappear in `#mode-switcher` instead (`002-componentes-layout.md`, "Header control row").

### Edit-mode header texture: `.app-title-bar--edit` (00283, simplified 00290)

Modifier class on `h1`, edit mode only (toggled from `ui/appTitle.js`, not CSS-derivable from `h1`'s own state otherwise):
...
Layers a subtle 135°-diagonal repeating stripe (8px period, `rgba(255,255,255,0.05)` — near-invisible, reinforcement only) over `h1`'s existing base gradient (`linear-gradient(180deg, var(--gray-800), var(--bg-toolbar))`, unchanged, repeated here since `background-image` on the modifier fully replaces the shorthand rather than composing with it). No new token — both gradient stops reuse `001-tokens-visual.md`'s existing `--gray-800`/`--bg-toolbar`. Reusable convention for any other edit-mode-exclusive surface that wants the same "textured while editing" cue instead of introducing a distinct pattern each time.

- [gotcha] between 00287 and 00289, Import/Export lived in a separate sibling element (`.edit-toolbar`) that tried to visually match this texture (same color, same pattern, same `background-attachment: fixed` phase-lock) — none of those attempts eliminated a visible seam at the boundary between the two elements in a real browser. 00290 removed `.edit-toolbar` and merged its content into `h1` itself (`.app-title__controls`, above): a single element only ever has one background layer, so the seam is structurally impossible rather than merely hidden. **Prefer merging into one element over trying to visually match two adjacent ones** when a textured/patterned background needs to look continuous.
