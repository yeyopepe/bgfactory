...
- IDs (`#mode-switcher`, `#content`, `#app-version`, `#app-title`): reserved for unique layout containers in `index.html`. Never for reusable components. (`#edit-toolbar` existed until 00290, removed once Importar/Exportar merged into `h1#app-title`.)
