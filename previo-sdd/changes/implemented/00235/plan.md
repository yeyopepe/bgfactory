...
- [x] **`src/ui/visualEditorModal.js` — helper `getEditorWorkArea()` que mide el hueco interior real.** Añadir una función local a `openVisualEditorModal` (junto a `getEffectiveCanvasMaxSide`, tiene en closure `content`, `toolbar`, `facesRow`, `adjustImageBtn`, `faces`). Devuelve `{ availWidthPerFace, availHeight }` en px:
  ...
  - **Desviación del plan (aprobada con el usuario durante la implementación):** `getEffectiveCanvasMaxSide()` se calcula **una sola vez por `renderFaces()`**, cacheado en un closure `let currentCanvasMaxSide`, y tanto `renderFace()` como el margen del botón "Ajustar imagen…" leen esa caché. El plan asumía medir "el render anterior aún montado", pero `renderFaces()` vacía `facesRow.innerHTML` al principio, antes del primer `renderFace()` — así que al calcular por cara, la primera cara de una 'carta' correría con `facesRow` ya vacío (rama de fallback `EDITOR_CHROME_V`) y la segunda con medición real → dos caras de tamaño distinto. Solución: `currentCanvasMaxSide = getEffectiveCanvasMaxSide()` se ejecuta **antes** de vaciar `facesRow` (mide el cromo de la cara del render anterior, como pretendía el plan) y su resultado se reutiliza para todas las caras del render.
...
- [x] **`src/ui/visualEditorModal.js` — reescribir las ramas `maximized` y `manualSize` de `getEffectiveCanvasMaxSide()`.** (líneas ~301-312). La firma sigue devolviendo un escalar. En ambas ramas:
  ```js
...
- [x] **`src/styles/main.css` — `.card-editor-modal .modal__content` como columna flex.** Añadir una regla scoped: `.card-editor-modal .modal__content { display: flex; flex-direction: column; }`. Mantiene `flex: 1; overflow-y: auto; padding: 1.5rem` heredados de `.modal__content`. Necesario para que `.card-editor-modal__faces` pueda crecer y centrar el lienzo en el hueco vertical sobrante. No afecta a otras modales (selector acotado a `.card-editor-modal`).
  - **Desviación del plan (aprobada con el usuario durante la implementación):** además, `.card-editor-modal--maximized` pasa de sólo `max-height: 90vh` a `height: 90vh; max-height: 90vh`. Sin altura determinada la modal maximizada queda content-sized (`.modal-overlay` la centra, no la estira), `content.clientHeight` reflejaría el layout previo al maximizar y `getEditorWorkArea()` mediría un hueco pequeño → el lienzo no crecería a casi pantalla completa (el `renderFaces()` del handler de maximizar corre una sola vez, sin segunda pasada de convergencia). Con `height: 90vh` el `flex: 1` de `.modal__content` tiene holgura real que repartir. `manualSize` no se ve afectado: sus handlers fijan `modal.style.height` inline, que gana a la clase.
...
- [x] Abrir el editor de un **tablero personalizado** con un diseño **apaisado** (ancho ≫ alto) y pulsar **"Maximizar"**: el lienzo llena casi todo el ancho interior de la modal (no ~42% del viewport como antes), su alto es `ancho / proporción`, y queda **centrado verticalmente** en el hueco — el espacio sobrante se reparte arriba y abajo, no todo debajo. No aparece barra de scroll en el cuerpo de la modal.
...
- [x] Abrir el editor de un tablero personalizado con un diseño **vertical** (alto ≫ ancho), maximizar: ahora la restricción limitante es el **alto** — el lienzo crece hasta casi llenar el alto interior y queda **centrado horizontalmente**, con el sobrante de ancho a los lados.
- [x] Abrir el editor de una **carta** (dos caras, proporción vertical), maximizar y redimensionar grande: las dos caras siguen **una al lado de otra y alineadas** (sin apilarse, fix 00233) y ahora crecen también en **alto** hasta el límite del hueco; el botón "Ajustar imagen…" entre ellas queda **centrado verticalmente** respecto al alto real del lienzo. Sin scroll.
- [x] Con la carta en proporción **apaisada** (p. ej. Poker horizontal 7:5 o "Libre" ancha), maximizar: las dos caras topan el **ancho por cara** disponible y quedan centradas verticalmente en el hueco.
- [x] Comprobar que el estado **por defecto** (editor recién abierto, sin maximizar ni redimensionar) se ve **exactamente igual que antes**: lienzo(s) a `CANVAS_MAX_SIDE` y botón "Ajustar imagen…" con su margen fijo del CSS.
- [x] Encoger la **ventana del navegador** a un tamaño muy pequeño con el editor maximizado: el lienzo no baja de `CANVAS_MIN_SIDE`; si a ese mínimo no cabe, aparece scroll vertical en el cuerpo de la modal (nunca se recorta ni se oculta el pie "Aceptar/Cancelar").
- [x] Mover y redimensionar un cuadro de texto / figura dentro de un lienzo agrandado: sigue funcionando igual (el `previewScale` nuevo se aplica de forma coherente a la posición y tamaño de los elementos).
- [x] Cerrar y reabrir el editor tras haberlo maximizado/redimensionado: arranca en el tamaño por defecto (fix 00233 intacto).
