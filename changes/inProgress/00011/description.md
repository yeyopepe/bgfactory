...
- `src/main.js` (líneas ~38-47) siembra hoy un componente `'cuadro-texto'` por defecto en cada carga; este es el punto a condicionar según haya o no estado guardado.
...
- El fichero final se genera hoy mediante `scripts/build.py`, que inlinea todo en un único `src/_output/index.html`; "guardar" en tiempo de ejecución debe producir una copia de ese mismo HTML con el estado embebido (p.ej. inyectando el JSON de `state.components` en el propio documento antes de descargarlo), reutilizando el mismo formato/deserialización que el autoguardado de localStorage.
