...
Añade lo del hightlight del mazo también en el modo edición
...
### Ampliación: el resaltado del mazo también en modo edición

El resaltado visual del mazo mientras se arrastra algo por encima (descrito arriba para modo juego) se añade también al modo de edición, donde ya existe hoy la posibilidad de arrastrar una selección de cartas sobre un mazo para meterlas — pero sin ningún resaltado previo, solo una pregunta de confirmación al soltar.

- El mazo se resalta igual (mismo aspecto que en modo juego) tanto si se arrastra una sola carta como si se arrastra una selección de varias cartas a la vez — no se distingue por cantidad, solo importa que lo arrastrado sean cartas y que solape con el mazo.
- La confirmación que ya existe en modo edición al soltar sobre un mazo (pregunta si se quiere meter la carta o cartas en él) se mantiene igual que hoy, sin cambios — el resaltado es únicamente un feedback visual añadido mientras se arrastra, antes de soltar; no sustituye ni afecta a esa confirmación.
- El resto del comportamiento de este cambio (posición de inserción al confirmar, casos límite de solape, etc.) sigue siendo el que ya tenía modo edición antes de este cambio — aquí solo se añade el resaltado durante el arrastre.

```mermaid
stateDiagram-v2
    [*] --> Reposo

    Reposo --> Arrastrando: se arrastra una selección compuesta solo por cartas

    state Arrastrando {
        [*] --> SinSolape
        SinSolape --> SobreMazo: la selección se solapa con un mazo
        SobreMazo --> SinSolape: deja de solaparse con ningún mazo
    }

    Arrastrando --> Reposo: soltar sin solape\n(comportamiento actual, sin cambios)
    SobreMazo --> Confirmando: soltar con solape

    Confirmando --> CartaInsertada: se confirma en el diálogo\n(comportamiento actual, sin cambios)
    Confirmando --> Reposo: se cancela el diálogo\n(comportamiento actual, sin cambios)

    CartaInsertada --> Reposo
    Reposo --> [*]
```
