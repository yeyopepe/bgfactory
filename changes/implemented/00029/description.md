...
- Un nuevo tipo de elemento debe registrarse en: `src/ui/componentTypeModal.js` (array `COMPONENT_TYPES`), `src/ui/componentModal.js` (`createDefaultComponent(type)` y `renderSpecificTab()`) y `src/ui/componentRenderer.js` (renderizado real en el tablero). Ver también `design/docs/ARCHITECTURE.md` §4/§5. Además de `'tablero'`, ya hay otros dos tipos registrados con este mismo patrón que sirven de referencia: `'dado'` y `'documento'`.
...
- El cambio `00020` ("Dado") ya está implementado y cerrado (`changes/closed/00020/`); sirve como precedente de patrón ya construido (no pendiente) para el modal de propiedades específicas (pestañas Generales/Específicas, campos `.modal__field`, `.modal__field--row`, botón que abre un submodal de selección de recurso).
- `design/docs/stylebible/STYLE_BIBLE.md` §13 prohíbe sombras, gradientes, bordes muy redondeados y animaciones salvo excepciones explícitamente documentadas; actualmente la excepción cubre el bisel de los tipos `'tablero'` y `'dado'` (cambios 00019 y 00020), más una nota aparte sobre el parpadeo/shake del dado — cualquier efecto similar para la ficha requeriría documentar una nueva excepción, no asumir las ya existentes.
