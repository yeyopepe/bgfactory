...
- No se toca el proceso de versión: `build_game.py`/`build_distribute_game.py` leen `CURRENT_VERSION` de `src/data/version.js` tal cual, sin incrementarla ni escribirla — eso sigue siendo exclusivo de `build_all.py`.
- No se modifica en ningún sentido `build_all.py` ni su comportamiento (sigue incrementando versión y generando la build completa igual que hoy).

**Dudas técnicas resueltas con el usuario en esta fase de planificación** (adicionales a las ya resueltas en `description.md`):

- **¿Qué separador usar en el sufijo `.obf` (el código actual usa `_editor.obf.html` con guion bajo, la ampliación describía guion)?** Unificar todo a guion: se renombra también la salida ya existente de `build_distribute_all.py` a `index-v{NNNN}-editor.obf.html`, y la nueva a `index-v{NNNN}-game.obf.html`. **Superado por la ampliación de alcance posterior** (ver más abajo, "Ampliación de alcance confirmada..."): el sufijo de la build completa pasa de `-editor` a `-all` (`index-v{NNNN}-all.obf.html`), en línea con el renombrado de `build.py`→`build_all.py`.
- **¿Cómo recibe `build_game.py` una partida ya diseñada (necesaria para que la limpieza de recursos tenga efecto real)?** Argumento opcional de línea de comandos `--partida <ruta>` apuntando a un HTML ya generado con el botón "Guardar" del modo edición (tiene `#initial-state` ya relleno). Sin ese argumento, se comporta como `build_all.py`: semilla vacía, sin recursos que filtrar.
...
**Incongruencia de documentación detectada** (vía `ms-internal-tech-analysis`, reconfirmada en una segunda pasada tras crecer la documentación técnica desde el análisis original): `design/docs/architecture/04-modes.md` describe `ui/editModeToggle.js` con dos funciones `renderEnterEditButton`/`renderEditToolbar`; el código actual expone en su lugar `renderModeSwitcher` (que incluye el botón de entrar en edición **y** el botón "Ajustar zoom") y `renderEditToolbar`. Se corrige en (c).

Además, la documentación de arquitectura pasó de un único `ARCHITECTURE.md` a varios ficheros (`design/docs/architecture/01..06-*.md` + `INDEX.md`) desde que se hizo el análisis técnico original de esta entrada — las referencias de la sección (c) se han actualizado a los ficheros actuales. El resto del análisis técnico (código de `main.js`, `editModeToggle.js`, `persistence.js`, `index.html`, `resource.js`, `globalShortcuts.js`, `build_all.py`, `build_distribute_all.py`) se ha reverificado contra el estado actual del código y sigue vigente sin cambios.

**Ampliación de alcance confirmada por el usuario durante la implementación** (`description.md` se editó en vivo mientras `ms-do` ya estaba trabajando en esta entrada): se renombra el antiguo `build.py` → `build_all.py` (salida `index-v{NNNN}-all.html`) y el antiguo `build_distribute_editor.py` → `build_distribute_all.py` (salida `index-v{NNNN}-all.obf.html`), para que los cuatro scripts sigan un esquema de nombres uniforme (`build_all`/`build_game` × sin ofuscar/`build_distribute_*`). El renombrado se propaga a toda referencia externa a esos dos scripts encontrada en el repo (confirmado explícitamente por el usuario, no solo `src/`): `.claude/ms-guide.md`, `framework/how-to-compile-version.md`, `.claude/settings.json` (permisos), `.vscode/settings.json` (tarea Python) y `README.md`. `design/docs/architecture/06-persistence-build.md`/`INDEX.md` (sección (c) de este plan) ya reflejan los nombres nuevos directamente, sin necesidad de mencionar aquí el nombre viejo.
...
flowchart TD
  subgraph SinOfuscar["Sin ofuscar"]
    B[build_all.py] -->|"index-v{N}-all.html"| BH[Build completa]
    G[build_game.py] -->|"index-v{N}-game.html"| GH[Build solo mesa]
  end
  subgraph Ofuscado
    DE[build_distribute_all.py] -->|"index-v{N}-all.obf.html"| DEH[Build completa ofuscada]
...
flowchart TD
  ...
  B -- no --> D[Semilla vacía, igual que build_all.py]
  ...
  H -- sí --> I["Recorrer grafo de imports desde main.js YA RECORTADO (visit_module/IMPORT_PATTERN, igual que build_all.py)"]
  I --> J[Transformar cada módulo superviviente require/module.exports]
  J --> K[Concatenar módulos + runtime, igual que build_all.py]
  K --> L[Incrustar CSS de main.css + assets referenciados, igual que build_all.py]
...
- Reutilizar literalmente de `build_all.py`: `MIME_TYPES`, `to_data_uri`, `embed_css_asset_urls`, `embed_html_asset_refs`, `resolve_module_path`, el sistema `require`/`module.exports` (`RUNTIME`), y la comprobación del marcador `{VERSION}` en el `<title>`.
- Salida: `src/_output/versions/index-v{NNNN}-game.html` (misma carpeta que `build_all.py`, `NNNN` = `CURRENT_VERSION` vigente, sin incrementar). El script imprime `Build de solo mesa generado en {ruta}` — nótese que, a diferencia del mensaje de `build_all.py`, **debe seguir conteniendo la subcadena** `Build generado en` en algún punto reconocible para que `build_distribute_game.py` pueda localizar la ruta con la misma regex que ya usa `build_distribute_all.py` (ver punto 5) — o, más simple, imprimir literalmente `Build generado en {ruta}` igual que `build_all.py`, sin cambiar el mensaje.

### 4. `src/scripts/build_distribute_all.py` (renombrado desde `build_distribute_editor.py`, más ajuste de sufijo)

Se renombra el fichero (antes `build_distribute_editor.py`) y se actualiza su referencia interna al script de build (antes `build.py`, ahora `build_all.py`). El nombre del fichero de salida pasa de `index-v{NNNN}_editor.obf.html` a `index-v{NNNN}-all.obf.html` (guion en vez de guion bajo, y sufijo `-all` en vez de `-editor` por la unificación de esquema de nombres acordada — ver ampliación de alcance en (a)). Sin cambios en la lógica de ofuscado ni de extracción.

```mermaid
flowchart TD
  A[Inicio] --> B[Ejecutar build_all.py]
  ...
  F --> G["Escribir index-v{NNNN}-all.obf.html"]
```

### 5. `src/scripts/build_distribute_game.py` (nuevo)

Análogo a `build_distribute_all.py`, reutilizando la misma `BUNDLE_SCRIPT_PATTERN` y el mismo `obfuscate_bundle.js` (sin cambios en este último — ya es agnóstico, recibe rutas de entrada/salida por argumento):
...
Acepta el mismo argumento opcional `--partida <ruta>` que `build_game.py` y lo reenvía tal cual al invocarlo como subproceso. Si `build_game.py` o el ofuscador fallan, se detiene con error claro sin generar un entregable a medias (mismo criterio que `build_distribute_all.py` hoy).
...
3. Renombrar el sufijo de salida de `build_distribute_all.py`.
4. Escribir `build_distribute_game.py`.

## (c) Cambios de arquitectura

- **`design/docs/architecture/06-persistence-build.md`** (sección "Desarrollo y build"): ampliar para documentar los cuatro scripts de build/distribución y su fichero de salida (tabla o lista análoga a la del diagrama de la sección (b) de este plan), y la convención de marcadores `EDIT-ONLY-START`/`EDIT-ONLY-END` con la lista de ficheros que los usan hoy (`main.js`, `ui/editModeToggle.js`, `core/persistence.js`, `index.html`), dejando constancia de la convención a mantener: cualquier funcionalidad nueva de modo edición debe preferir un fichero propio (detectado solo por alcanzabilidad de grafo) en vez de mezclarse inline en uno de estos cuatro.
- **`design/docs/architecture/04-modes.md`** (línea 5, incongruencia detectada en (a)): sustituir la referencia a `renderEnterEditButton` por `renderModeSwitcher` (que incluye también el botón "Ajustar zoom", ya documentado por separado en esa misma sección).
- **`design/docs/architecture/INDEX.md`**: si la ampliación de `06-persistence-build.md` cambia de forma relevante lo que resume la fila de la tabla "Ficheros hermanos" para ese fichero, actualizarla también.
